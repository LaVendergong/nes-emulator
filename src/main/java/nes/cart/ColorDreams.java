...
/**
 * Mapper 11（Color Dreams）。写 $8000：低位 32K PRG，高位 8K CHR。
 */
final class ColorDreams implements Cartridge {
    ...
    ColorDreams(byte[] prg, byte[] chr, boolean chrRam, boolean verticalMirroring) {
        this.prg = prg;
        this.chr = chr;
        this.chrRam = chrRam;
    ...
    public void cpuWrite(int address, int value) {
        ...
        value &= 0xFF;
        if (address >= 0x8000) {
            int prgBanks = Math.max(1, prg.length / 0x8000);
            int chrBanks = Math.max(1, chr.length / 0x2000);
            prgBank = (value & 3) % prgBanks;
            chrBank = ((value >> 4) & 0x0F) % chrBanks;
            return;
        }
        if (address >= 0x6000) {
    ...
    public void loadState(java.io.DataInput in) throws java.io.IOException {
        if (in.readInt() != 11) {
            throw new java.io.IOException("存档不是 Color Dreams");
        }
