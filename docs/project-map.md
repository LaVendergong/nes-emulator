...
NTSC FC/NES 模拟器：你提供 ROM，Windows 或 Android 把游戏跑到约 60 帧。
...
4. 采样率 44100：N 个 CPU cycle 产出约 `N * 44100 / 1789773` 个采样（误差 ≤1）。`drainTo` 装不下的采样必须留下，不能丢。 — 验证：同上
...
Windows 发布：解压 `dist/fc-nes-*-windows.zip`，双击 `FC-NES.exe`。存档写在 exe 旁的 `saves/`。
